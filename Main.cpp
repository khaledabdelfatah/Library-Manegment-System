...
///START OF PROG
    ...
    do
    {
        ...
        if(User_or_admin_choice==1)
        {
            ...
            if(login.Is_Loged(AdminUserName,AdminPassword,"Admin.txt"))
            {
                Admin_is_log=true;
            }
        ...
        else if(User_or_admin_choice==2)
        {
            ...
            if(User_log_reg==1)
            {
                ...
                if(login.Is_Loged(User_user_name,User_Password,"User.txt")==true)
                {
                    User_is_log=true;
                }
            }///END OF lOGIN
...
////////////////////////////////////////////////////End Login And Registeratiom ///
        if(Admin_is_log)
        {
            ...
            do
            {
                ...
                {
                    Admin_work.Manage_Borrow_system();
                }
                else if(Admin_choice==3)
                {
                    Admin_work.User_Request();   ///See Users Request For New Book
                }
                else if(Admin_choice==4)
                {
                    Admin_work.Users_feedback();   ///user feedback
                }
        ...
        if(User_is_log)
        {
            ...
            do
            {
                ...
                else if(User_choics==4) ///Borrow
                {
                    ...
                    if(borrowed_cat=="Book_computer science.txt")
                    {
                        borrowed_notreal="computer science.txt";
                    }
                    else if(borrowed_cat=="Book_cooking.txt")
                    {
                        borrowed_notreal="cooking.txt";
                    }
                    else if(borrowed_cat=="Book_economic.txt")
                    {
                        borrowed_notreal="economic.txt";
                    }
                    else if(borrowed_cat=="Book_philosophy.txt")
                    {
                        borrowed_notreal="philosophy.txt";
                    }
                    else if(borrowed_cat=="Book_science.txt")
                    {
                        borrowed_notreal="science.txt";
                    }
                    else
                    {
